#!/usr/bin/env tsx
/**
 * Drift detection for Surinamese Law MCP.
 *
 * Checks if upstream Surinamese law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Surinamese Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'surinamese-law-mcp/1.0 drift-detect' },
      });
