#!/usr/bin/env tsx
/**
 * Surinamese Law MCP -- Data Freshness Checker
 *
...
const PORTAL_URL = 'https://wetten.sr';
const PORTAL_NAME = 'Surinamese Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/surinamese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Surinamese Law MCP — Data Freshness Check');
