#!/usr/bin/env tsx
/**
 * Database builder for Surinamese Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Surinamese Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Surinamese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'SR');
    insertMeta.run('source', 'wetten.sr + dna.sr');
